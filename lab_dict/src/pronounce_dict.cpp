...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    vector<string> pronounciation1;
    if (dict.count(word1_upper)) pronounciation1 = dict.at(word1_upper);
    vector<string> pronounciation2;
    if (dict.count(word2_upper)) pronounciation2 = dict.at(word2_upper);
    // not sure if this works....
    if (pronounciation1.empty() || pronounciation2.empty()) return false;
    return std::includes(pronounciation1.begin(), pronounciation1.end(), pronounciation2.begin(), pronounciation2.end());
