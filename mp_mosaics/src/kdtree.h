...
class KDTree
{
  ...
  private:
    ...
    KDTree<Dim>::KDTreeNode* build(unsigned left, unsigned right, unsigned d);
    Point<Dim> quickselect(unsigned left, unsigned right, unsigned currDim, unsigned mid);
    unsigned partition(unsigned left, unsigned right, unsigned dim, unsigned pivotIdx);
    typename KDTree<Dim>::KDTreeNode* _copy(KDTreeNode * subroot);
    void _delete(KDTreeNode * subroot);
    KDTree<Dim>::KDTreeNode* _find(unsigned dim, const Point<Dim>& query, KDTreeNode* subRoot, KDTreeNode* currentBest) const;
};
